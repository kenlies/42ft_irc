...
class MODE : public ACommand {
	public:
		...
		std::string const	arranger(std::string const message, Channel *targetChannel) const;
		std::string const	arranger(std::string const message, Client *targetClient) const;
	...
	private:
		...
		std::string const	parseModeMessage(std::string modeString, std::vector<std::string> const modeParameters);
};
